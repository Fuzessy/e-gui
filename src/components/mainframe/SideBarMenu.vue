<template>
  <div class="menu">
    <div v-for="(menuElem, index) in menuStructure" :key="index">
      ...
      <div
        class="subMenu"
        :class="menuElem.submenuHidden ? 'submenuHidden' : ''"
      >
        <div
          ...
          class="sub-menu-item"
        >
          <div @click="navigateTo(submenuElem)" class="submenu-item">
            {{ submenuElem.label }}
          </div>
        </div>
      </div>
...
export default class SideBarMenu extends Vue {
  ...
  mounted() {
    this.menuStructure = [];
    axios.get<Array<Menu>>("http://localhost:9999/process").then(response => {
